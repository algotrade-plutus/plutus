...
- **✅ [D71](#17-known-defects-and-docstring-mismatches) FIXED (2026-08-29).** D71 itself is
  closed — no committed live defect from D71 remains. (Related, now resolved: **MUST #4**
  (daily-VM cash settlement) has **landed** 2026-08-29 — `settle_daily` wired into
  `exchange._overnight_margin`; strategies **S1/S7** and the `test_margin_incidence_account`
  test, red on the in-progress tree, were updated to the landed behaviour and are green again;
  see the Suite line below and `PUBLISH-CHECKLIST.md`.)
  The one defect D71 fixed: on a **tick** run the ATC filled at
  `state.last`, a **stale pre-auction print**, instead of the day's **published close** the model
  takes as the ATC outcome (A75, because auction-window tick data is untrustworthy and the close
  is already stored). The one-condition fix landed in `exchange._interval_for`: in an auction
  phase a **synthesised** interval drops the close and names it missing — exactly as the open
  already is — so the ATC now returns the published close where a source supplies one and
  **INDETERMINATE** on a bare snapshot, symmetric with the ATO and never the stale last. Guarded
...
- **Suite:** collected per-suite under the venv (`.venv/bin/python -m pytest`), 2026-08-29:
  **`tests/market` 1646 · `scenarios` 38 · `strategies` 13.** `scenarios` is fully green.
  **Caveat on market/strategies pass count:** **MUST #4 (daily variation-margin) has landed
  ...
  updated to the landed behaviour and re-run green on 2026-08-29: strategies **S1**/**S7**, and
  the market test formerly named `test_daily_rebaselining_without_the_cash_leg_loses_the_whole_loss`,
  now `test_margin_incidence_account.py::test_daily_cash_settlement_now_captures_the_loss_the_rebaseline_alone_lost`
  (31-test file green; the settled path captures the loss, `called` 9 → 44). Committed `HEAD`
  (`13cf9c1`) recorded them green before the WIP churn. Still re-measure the full market +
  strategies suites against a clean HEAD and quote per-suite pass/fail. Collect the three suites separately (a root-level `--collect-only` aborts on
  `fastmcp` in `tests/test_mcp`, and `testpaths` covers `tests/` only). The
  `tests/datahub/test_cli.py` subprocess tests a *bare* `python -m pytest` once reported as **17
  failures pass under the venv** — outside the venv the child could not import `plutus` (a
  PYTHONPATH / module-name collision at collection time), **not a logic failure**. On the
  collected-vs-passed gap recorded as **D36**: it was measured on the 1318-test tree and has not
  been re-measured since — do not quote D36's numbers against this one
...
- **Last surveyed:** **2026-08-27 (later still)** — the auction-fidelity read and a full
  pointer sweep. Four results, all in §19's last entry: **A75** replaces a citation that was
  the wrong article — the auction price is **our modelling choice**, the day's published
  close for the ATC and published open for the ATO, adopted because auction-window tick data
  is untrustworthy and both are already stored; **D71**, above; **A64 settled from the code** against
  `PUBLISH-CHECKLIST.md`'s RESOLVED entry; and **two whole modules that were missing from
  this inventory** — `adapters/depth.py` and `session/book_walk.py`, which between them end
  the "the corpus has no order-book sizes" claim. Also **~45 `file:line` pointers repaired**;
  the drift in `exchange.py` and `orders.py` was systemic, not two typos. Prior survey
  2026-08-26, by five readers of the actual code (not of docstrings)
...
| "When may an order be amended?" | Price *and* quantity together allowed to 2025-05-04, **forbidden from 2025-05-05** (VNX QĐ 22/2025 Điều 21.3) | `exchange.py::_may_amend_price_and_quantity` (`:2108`; both citations in its docstring at `:2112-2114`) |
| "What price does an **auction** fill at?" | The **published open (ATO) / close (ATC)**, and that is **our modelling choice, not a citation** — see A75. We do not trust the tick data inside the auction window, so we take the day's published open/close, both already in the database. No Vietnamese instrument says the published close is the ATC cross, and **none defines an opening price at all**. QĐ 352 Điều 2.5 (close = the day's last match) is *context* for why the close is a fair stand-in; the price rule itself is ours | `fills.py::auction_fill_price` (`:608-636`), A75 |
...
| A69 | On a daily run the fill interval is `[ts, ts + 1 day)` — the **whole trading day** | `exchange.py::_interval_for` (`:2435`) | **Declared look-ahead.** An order entered at 14:00 is evaluated against the whole day. "An over-generosity that is a declared consequence of the resolution and not something a fill policy may silently correct." Present in **every** daily-resolution fill |
...
| A75 | **The auction cross price IS the published open / close.** `auction_fill_price` returns `interval.open` in the ATO phase and `interval.close` in the ATC phase | `fills.py::auction_fill_price` (`:608-636`) | **OUR MODELLING CHOICE — a stated approximation, added 2026-08-27, and it replaces a citation that was fiction.** We cannot trust the tick data inside the ATO/ATC auction window, so we take the day's **published close** as the ATC outcome and the day's **published open** as the ATO outcome — both already in the database. That is the whole of it: a modelling choice, honestly noted, with no rule behind the price. The row this supersedes attributed the behaviour to QĐ 352 **Điều 6.2(a)/6.3**; both are the wrong article — 6.2(a)–(d) is the four-step algorithm that DERIVES a cross **from a book** (rulebook §2.4:190), and 6.3 is the **continuous**-session resting-price rule — and **we deliberately cite neither for the price**. As *context* only, and not as the rule, QĐ 352 **Điều 2.5** (*"Giá đóng cửa là giá thực hiện tại lần khớp lệnh cuối cùng trong ngày giao dịch"* — the close is the day's **last match**; carried into VNX QĐ 17 Điều 3.17 and from 2025-05-05 QĐ 22/2025 Điều 3.17) is why the published close is a fair stand-in for the ATC cross. Separately, there is **no opening price in Vietnamese exchange regulation at all** — nothing defines one and nothing consumes one, because the next-day reference is the close — so `interval.open` is a **vendor construct**, and the ATO half likewise rests on a stored vendor field with no rule behind it. ⚠️ Not yet self-declared in `fills.py`: `auction_fill_price` grounds itself in *"Design section 8"* (`[design]`, correct) but carries no UNSOURCED tag naming this modelling choice — §18 rule 1 asks for the docstring and this register to agree, so that is a code edit this pass did not own |
...
| A40 | Daily-bar session phase = `CONTINUOUS` | **Scoped to `Resolution.DAILY` only.** Declared deviation from the interface contract; both shipped adapters hardcode it (`datahub.py:436`, `tick.py:109`), and on a daily run `_phase` (`exchange.py:2081-2101`) prefers that adapter value. **On any other resolution the rulebook wins outright** and the adapter is only the `UNKNOWN` fallback — so ATO/ATC/PLO *are* reachable on a tick run. Also: on a daily run a non-trading day resolves `POST_CLOSE`, which `equity.py:183-185` refuses with `SESSION_SEMANTICS`. **CORRECTED 2026-08-27 (later) — the previous qualification is WITHDRAWN.** This row used to claim *"the fill does not go through `_phase` at all"* and that `interval.session` is the adapter's hardcoded value **at every resolution**, and therefore that **no ATO or ATC has ever crossed on a shipped source**. Re-read line by line, that is false: `_evaluate_fills` resolves `phase = self._phase(record.venue, observed=state.session)` at **`exchange.py:2841`** and hands `_interval_for` a `replace(state, session=phase)` at **`:2842-2843`**, so on a tick run the interval's phase *is* the rulebook's. The two halves now split like this. **The ATO half is still right.** `TickSource` is not an `IntervalSource` (`tick.py` has `states` `:55` and `state_at` `:159`, no `interval()`), so `_interval_for` synthesises at `:2442-2451`; `OPEN` is always in `missing` and `interval.open` is never set, so `auction_fill_price` returns `None` and an ATO never crosses — which is the right answer, because at that resolution we have no published open to hand it (A75). **The ATC half crossed, and violated the stated model — FIXED 2026-08-29 (D71, commit `74e667a`).** The design takes the published close as the ATC outcome (A75), but on a tick run `close = state.last` was a **pre-auction print** and `CLOSE` was named missing only when `state.last` was `None`, so the ATC filled at `state.last` rather than the published close — the design-conformance defect **D71**. The fix makes a **synthesised** interval in an auction phase drop the close and name `DataField.CLOSE` missing (`exchange.py:2648-2668`), so the ATC now returns the published close where a source supplies one and INDETERMINATE otherwise, symmetric with the ATO. **The daily path remains inert, by an accident worth knowing about:** `DataHubSource` is an `IntervalSource`, so `_interval_for` returns its **own** interval at `:2441` and the `:2843` re-stamp is discarded before it can be read — which means the seam hazard `validation/scenarios/bars.py:375-381` warns of is real but currently masked, and unmasks itself the moment any adapter serves intervals at tick resolution. Tracked as a SHOULD in `PUBLISH-CHECKLIST.md`; that file's wording needs the same correction and is not edited from here |
...
| A60 | ~~VM measured from `average_entry` for the life of a position~~ **RESOLVED 2026-08-29 (MUST #4, D1).** The baseline now rolls daily | `settle_daily` is wired into `_overnight_margin` (`exchange.py:4026`), rolling the VM baseline to each day's DSP and settling the day's P&L in cash, so VM is the **day's** adverse move from the previous DSP, not the cumulative since-entry loss |
...
| # | Behaviour | Why assumed |
|---|---|---|
| A64 | Default settlement calendar `weekday-only-UNSOURCED` | **No calendar data ships.** Every default run's settlement dates are wrong around Tết — and, more precisely than this row used to say, around **every closure the exchange takes**, Tết being merely the largest and the one that is measured. Visible only via `provenance().settlement_calendar_id`. **Settled from the code 2026-08-27, third reading, and A64 stands.** Three facts, each greppable: (1) **the default path.** `ExchangeSession.build` (`exchange.py:1052`) calls `_settlement_calendar` (`:1297`), whose last line `:1331` is `return supplied if supplied is not None else weekday_settlement_calendar()`. `weekday_settlement_calendar` (`calendar.py:783`) constructs with `frozenset()` holidays and the id at `calendar.py:779`, and `is_settlement_day` (`calendar.py:390`) is then `day.weekday() < 5 and day not in self._holidays` over that empty set. **A user who supplies nothing gets the weekday-only calendar, and it is the wrong one.** Its own docstring says so: *"wrong by construction … it settles on Tết, on 30 April, on 2 September"*, and `is_sourced` is `False` precisely so a caller can refuse it. (2) **the derived path is not the default and has no caller in `src/`.** `grep -rn from_settlement_days src/` returns **nothing**; the only call sites are `validation/scenarios/equity-margin.py:254` and `tests/market/session/test_calendar.py`. The two supported non-default routes are `data.settlement_calendar` in the config (loaded by `VsdcSettlementCalendar.from_file`, `exchange.py:1330`) and a `settlement=` object passed to `build()`; naming both raises. (3) **the derived path does not reproduce 4228 either, and the harness says so itself.** `validation/scenarios/settlement.py`'s `CALENDAR_SOURCE` (`:134-143`) calls the corpus-measured closure set *"an **INFERENCE**, not a VSDC notice"* and states outright that *"The rulebook grades the settlement calendar on Announcement 4228/TB-VSDC (2025-11-20), which is outside this coverage; this calendar **inherits none of that grade**"* — the measured window ends 2022-12-30 and 4228 is dated 2025-11-20, so days-the-data-carries cannot reproduce it even in principle. What reproduces 4228 is `from_file` **fed the notice**, which is the caller-supplied route and ships no data. Measured error of the default, from our own docstring at `exchange.py:1306-1307`: a 2026-02-12 trade answers **T+2 = 2026-02-16 where VSDC settled 2026-02-23 — five counted days the depository was shut**. **`PUBLISH-CHECKLIST.md`'s RESOLVED entry on this item is wrong on both halves** — the derived path is not what a default run takes, and it does not reproduce 4228. That file is another agent's this phase and is **not** edited from here; the correction is requested, not applied |
| A65 | Default trading calendar `weekday_trading_calendar()` | Same shape, same problem |
| A66 | `VnTradingCalendar` clock times fall back to the **undated** `core/constant.py` specs **only when the caller passes no `RuleSet`** | **Corrected — the earlier wording was wrong.** `calendar.py:720-731` / `:733-743` **prefer** `rules.session_open(venue)` / `rules.session_close(venue)`, and both instants the session actually stamps pass a real `RuleSet`: DAY-order expiry (`exchange.py::_expire_boundaries`, `:2696`, stamping through `::_session_close` `:2731`) and a margin call's `cure_by` (`exchange.py::_mark_derivatives` `:3398`, the deadline read at `:3477-3493` → `MarginMonitor.on_mark` → `deposit.py:1767`). `rulebook.py:2102-2130` says outright that `session_open` "exists because `calendar.py` was looking for it". So the undated map is a **fallback for a rules-less caller**, not the live path |
...
**Register total: 75 assumed values (A1–A75).** *(A72–A74 added 2026-08-27 with the
execution-half repair: the `soft` cap ambiguity, and the overnight layer's two. **A75 added
2026-08-27, later**: the auction cross price, which was carried as SOURCED against a citation
that turned out to be the wrong article.)* Every one says so in its own docstring or
`PROVENANCE` dict, with **two** logged exceptions: **A63**, missing from `margin.py`'s
`PROVENANCE` (D26), and **A75**, whose site `fills.py::auction_fill_price` names its
`[design]` origin but does not yet carry the UNSOURCED tag (see A75's own ⚠️).
...
| `SETTLEMENT`, pre-2022-08-29 branch — `delivery_on_next_session_open=True` | `:1295-1305`, `types.py:2193`, `calendar.py:425-479` | IMPLEMENTED + SOURCED | **A different rule, not just a different clock.** 2016-01-01→2022-08-26 settlement completed 15:30–16:00, *after* the close, so the first usable session is the **open of T+3**. `settles_at` **raises `CalendarError`** rather than guessing when no `TradingCalendar` was supplied. Wired at `exchange.py::_settles_at` (`:3275`). A run over 2020–2022 uses this branch, not the 13:00 one (A50 covers only the post-2022 branch) |
| `SETTLEMENT`, futures | `:1329-1346` | IMPLEMENTED + SOURCED | `(InstrumentKind.FUTURE,)` → T+1, HIGH, VSDC *"Bù trừ và Thanh toán"*. The row's own note: this is **daily variation margin settling T+1, not the contract cash-settling T+1**. The deposit **now does accumulate mark-to-market** — once a day, via `settle_daily` wired into `_overnight_margin` (MUST #4, 2026-08-29; **D1 resolved**), which was the missing counterpart this note used to flag |
...
| 1 | Venue configured for this session | `exchange.py::submit` (`:1500-1506`) | IMPLEMENTED + SOURCED | `[design]` — config, not a market rule |
| 2 | Dated order-type legality | `exchange.py::_legal_here` (`:2119`) | IMPLEMENTED + SOURCED | `rules.legal_order_types(venue, phase)`. HIGH except HOSE post-KRX MEDIUM |
| 3 | Dated per-order size cap | `exchange.py::_size_here` (`:2159`) | PARTIAL | A39 — an UNKNOWN cap refuses nothing |
| 4 | Dated tick resolution, pinned into a per-submission judge | `exchange.py::_dated_tick` (`:2215`), `rulebook.py:2623` | IMPLEMENTED + SOURCED | HNXDS 1 VND on 100,000đ face (HIGH); HNX ETF 0.001 from 2022-03-31 |
...
| 11 | Reservation (`_reserve`) — runs **around** `admits()`, never inside | `exchange.py::_reserve` (`:2512`) | IMPLEMENTED + SOURCED | `[design]` §7.0 |
...
| — | Segregation annotation on funding refusals | `exchange.py::_annotate_segregation` (`:2605`) | IMPLEMENTED + SOURCED | Rulebook 6.3, HIGH. Carries `funded_in_aggregate`, `auto_transfer=False` |
...
| — | Put-through (`Side.CROSS`) | `exchange.py::_reserve` (`:2578-2583`) | DEFERRED | Raises `ValueError`. The `PUT_THROUGH` tick axis exists but no admission path uses it |
...
| Feature | Where | Status | Source / note |
|---|---|---|---|
| 7 states, 4 terminal, single enforcement point | `types.py:273-375`, `orders.py:268` | IMPLEMENTED + SOURCED | `[design]` §12. No `PARTIALLY_FILLED → RESTING` edge, deliberately |
...
| Single terminal funnel `_terminate` → release hook fires exactly once | `orders.py::_terminate` (`:592`) | IMPLEMENTED + SOURCED | `[design]`. Reservations are still on the record when the hook fires |
| `convert_residue` — MTL leftover becomes a LIMIT | `orders.py::convert_residue` (`:761`) | PARTIAL | The **price is the caller's**; the module stays neutral because the derivatives residual price is a recorded CONFLICT (A51) |
| `cancel(order_id, ts, phase=)` | `orders.py::cancel` (`:880`) | PARTIAL | `phase=None` means the lock is **not evaluated at all**. **No `tif.rests` guard** — an `ACCEPTED` MOK is cancellable |
...
| `amend()` — quantity **decrease** only at Tier 1 | `exchange.py::amend` (`:1599`), `orders.py::amend` (`:935`) | PARTIAL | Price amends and amend-ups refused. Stated reason: re-taking a reservation on the same key is refused by design, and release-then-retake can leave a live order unfunded |
| Price+quantity-together ban, dated | `exchange.py::_may_amend_price_and_quantity` (`:2108`, citations `:2112-2114`) | IMPLEMENTED + SOURCED | Allowed to 2025-05-04 (QĐ 17 Điều 22.3); barred from 2025-05-05 (VNX QĐ 22/2025 Điều 21.3, verbatim). **Pointer corrected 2026-08-27** — `exchange.py:1306` was cited here and in §1 and is the **settlement-calendar** docstring (the Tết example A64 quotes). The instrument was right in both rows; only the pointer was stale, which is the shape §18 rule 6 exists for |
| Priority survives a **pure quantity decrease** and nothing else | `orders.py::amendment_preserves_priority` (`:358`) | IMPLEMENTED + SOURCED | VNX QĐ 17 Điều 22.3 read verbatim, effective 2022-03-31. Vietnam has exactly two priority levels: price then time |
| `expire_due` at session boundaries | `orders.py::expire_due` (`:1080`) | PARTIAL | Immediate families deliberately **not swept in this module** — a live MOK at a boundary is left visible rather than killed by an invented rule. **True of `orders.py` in isolation, false of the session:** `ExchangeSession._sweep_non_resting` (`exchange.py::_sweep_non_resting`, `:2746`, called from `::_session_close` `:2731`) expires every non-resting type at the day's close with its **own** trigger via `_CLOSE_TRIGGER_BY_TIF` (`exchange.py:272-276`) — ATO/ATC → `AUCTION_CROSS`, MOK → `NOT_FILLABLE_IN_FULL`, MAK → `IMMEDIATE_REMAINDER` — explicitly so a reservation is not held for the rest of the run. **MOK reservations do not leak overnight** |
| Order ids `PLU-%08d`, lexical order = issue order | `orders.py::OrderIdFactory` (`:400`) | IMPLEMENTED + SOURCED | Justified by rulebook 2.4. Breaks silently past 99,999,999 |
| Amendment does **not** update encumbrances or re-run admission | `orders.py::amend` (`:935`) | NOT BUILT | Undocumented in `orders.py`; `exchange.py` wraps it, but the module in isolation permits an under-reserved amended order |

---

## 7. Fills

`session/fills.py` (**2,630 lines**). Three shipped policies behind one seam — **plus a fourth, `BookWalkFillPolicy`, which lives in `session/book_walk.py` and is deliberately not config-buildable (§14).**
...
| Fill price, **continuous** — the order's **own limit** | `fills.py:48-73` (convention 1) | IMPLEMENTED + SOURCED | **The strongest citation in the module**: Vietnamese matching trades at the **resting** order's price — QĐ 352 Điều 6.3, HIGH. Note §16.4 item 3: for an *aggressor* that citation says the opposite of what the code does |
| Fill price, **auction** — the published open (ATO) / close (ATC) | `fills.py::auction_fill_price` (`:608-636`), reached through `::_clearing_price` (`:2465-2473`) | IMPLEMENTED + **ASSUMED** | **Re-graded 2026-08-27. This row said SOURCED and shared the Điều 6.3 citation above; it has no citation of its own and never did.** The behaviour is **our modelling choice** — the day's published open/close, adopted because auction-window tick data is untrustworthy and both are already stored (A75). The previously recorded QĐ 352 **Điều 6.2(a)/6.3** are both the wrong article — 6.2(a)–(d) is the four-step algorithm that DERIVES a cross **from a book**, and 6.3 is the **continuous**-session resting-price rule — and we deliberately cite neither for the price. `auction_fill_price`'s own docstring now grounds it in *"Design section 8"*, i.e. `[design]`, which is not market law. **A75** carries the full statement; there Điều 2.5 (close = the day's last match) is noted as *context* for why the published close is a fair stand-in, and **no Vietnamese instrument defines an opening price at all** |
...
| **18** distinct `INDETERMINATE` return sites | `fills.py:424, 826, 836, 862, 1027, 1041, 1064, 1222, 1238, 1251, 1276, 1703, 1716, 1726, 1753, 2596, 2606, 2624` | IMPLEMENTED + SOURCED | `[design]` §8 — returning INDETERMINATE rather than guessing is the design decision; **the count itself is a code fact with no source, and must be re-derived, not cited.** **There is no catalogue.** An earlier revision of this row cited an artefact "F-I1…F-I20" that has never existed anywhere in the repo (`grep -r "F-I" src/ tests/ docs/` hits only this file) and gave the count as 20. Re-derive with `grep -n "FillDecision\.indeterminate(" src/plutus/market/session/fills.py`. **Four name no `DataField`** — `:1064, :1276, :1753, :2624` pass a literal `()`, because the missing thing is a rulebook fact or an unrecoverable queue position. Three more (`:862, :1238, :1716`) pass `[field] if field is not None else ()`, i.e. empty only when the interval carries no price field at all |
...
| Allocation **at** the marginal auction price | `fills.py::HardFillPolicy` (`:1276`, the site that names no `DataField`) | DEFERRED | Sourced as an **absence** — rulebook 2.4 records it UNVERIFIED. Returns INDETERMINATE rather than guessing. **This deferral is still right and its reason is unchanged: the missing thing is a RULE, not data.** Do not let §14's standing correction (2) be read as retiring it — sizes make computing *a* cross possible; they do not tell us how Vietnam rations at the marginal price, and drawing a number for an unwritten rule stays worse than drawing one for an unobservable queue position |
| Queue-position matching against a reconstructed book | `session/book_walk.py`, `adapters/depth.py` | **PARTIAL — no longer DEFERRED, corrected 2026-08-27** | The old reason ("no order ids exist in the corpus, `probabilistic` is the honest ceiling") is **half spent**. It is still true that no order ids exist and that our *own* queue position is unrecoverable — that is why `BookWalkFillPolicy` takes an explicit `queue=` assumption rather than deriving one. What is **no longer true** is that the depth behind the touch is unavailable: `DepthSource` serves it with sizes (§14, standing correction 2), and a marketable order now walks levels 1–3 producing one tranche per level at the resting level's own price. See §14 for both modules |
| The interval a policy is handed on a daily run is the **whole day** | `exchange.py::_interval_for` (`:2435`) | IMPLEMENTED + ASSUMED | **A69 — declared look-ahead.** Not a fills.py behaviour, but it conditions every daily-resolution decision this table describes |
...
| T+2 settlement, dated, with the 13:00 delivery regime | `rulebook.py:1266`, `exchange.py::_settles_at` (`:3275`) | IMPLEMENTED + SOURCED | **T+2 since 2016-01-01.** HIGH. **Only QĐ 109 Art. 4(4) was read verbatim**; QĐ 211 is known from Decision 109's own preamble (`rulebook.py:1293-1294`) |
| **Pre-2022-08-29 regime: delivery at the next session's open** | `rulebook.py:1295-1305`, `calendar.py:425-479`, `exchange.py::_settles_at` (`:3275`) | IMPLEMENTED + SOURCED | Settlement completed 15:30–16:00, after the close, so T+3's open is the first usable session. `settles_at` **raises `CalendarError`** rather than guessing when the caller supplied no `TradingCalendar`. **Any run over 2020–2022 is on this branch**, which A50 does not cover |
...
| Sale advance (*ứng trước tiền bán*) — legal status and formula | `ledgers.py:596-1401` | PARTIAL | Two halves, two grades. **Legal status** — Luật CK 54/2019 Art. 86(1)(b): rulebook **§5.2:541**, **`high`** (*"confirmed word for word"*). **Cap ≤ 1**, because a securities company may not lend — TT 121/2020 Art. 27: rulebook **§5.2:542**, **`low`** (*"read in summary form only, not verbatim"*). **Grade the second half `low`.** Separately, **`ledgers.py:628, :726, :1221` all cite "rulebook 8.4", which does not exist** — §8 runs 8.1 Taxes / 8.2 Exchange and depository prices / 8.3 Brokerage commissions and stops (D28, which also carries the per-site split; the sites were recorded as `:1209` and the rulebook row as `:511`, both stale) |
...
| Settlement calendar as a **caller-supplied data input** | `calendar.py:140`, `::from_file` , `::from_settlement_days` (`:233`) | PARTIAL | Mechanism reproduces the rulebook's Tết-2026 worked example **when fed the notice**; **no calendar data ships** (A64). **"Caller-supplied" is the correct path and not the default one** — a caller who supplies nothing gets `weekday_settlement_calendar()` (`exchange.py:1331`), not a derivation, and `from_settlement_days` has **zero callers in `src/`**. Read A64 before quoting this row as a resolution |
...
| Forced liquidation | `exchange.py::_mark_derivatives` (`:3398`; the `FORCED_LIQUIDATION` branch is `:3502`) | PARTIAL | **Reports only.** `FORCED_LIQUIDATION` carries `detail['executed'] = False`, the selection rule and the sequence. Nothing is closed |
...
| Daily settlement rebaseline (`settle_daily`) | `:1262-1317` | IMPLEMENTED + SOURCED | **Wired 2026-08-29 (MUST #4).** Called from `exchange.py::_overnight_margin` (`:4026`) once per settlement day after the close: it settles the day's P&L in cash (`_move`) and rolls the baseline to the DSP, both regimes — so VM is the day's move, not cumulative-since-entry (**D1 resolved**). Skipped only when a held contract has no settlement price that session. VSDC "index futures daily variation margin T+1"; QĐ 26 Điều 20 |
...
`session/exchange.py` (**4,041 lines**; the recorded 3,866 predates the overnight seam). `ExchangeSession`, aliased `Session`.

| Feature | Where | Status | Source / note |
|---|---|---|---|
| `submit` / `cancel` / `amend` / `orders` / `poll` | `:1453`, `:1570`, `:1599`, `:1647`, `:1659` | PARTIAL | `amend` is decrease-only (§6). `cancel` on an unknown id raises `KeyError`, not `Rejected` |
| `advance_to(ts)` — monotone clock, fixed internal order | `:1362` | IMPLEMENTED + SOURCED | `[design]` — no Vietnamese document orders a simulator's internal passes. Ours: expire → fill → decide immediates → settle → accrue → mark derivatives → **overnight margin** → equity margin → drain. The overnight step runs **after** the mark and after expiry settlement, and the order is load-bearing: a contract that cash-settled today is not carried past tonight's close, so the end-of-day book has to be the post-expiry one |
| `holdings` / `cash` / `positions` / `margin` / `charges` | `:1679-1761` | IMPLEMENTED + SOURCED | `[design]` — a scope decision, not a market rule. Read models only. No P&L, no portfolio — that is the caller's |
| `transfer(source, destination, amount)` | `:1763` | PARTIAL | Arrival immediate (A55). Refusals are counted but **emit no event** |
...
| `provenance()` — rulebook id, fill-policy signature, pins, calendar id, liquidation rule | `:1812` | IMPLEMENTED + SOURCED | `[design]`. Pins are always reported, so a counterfactual run self-declares |
...
| 12 `EventKind` members, all reachable; monotone `seq`; destructive single-consumer cursor | `types.py:779`, `exchange.py::poll` (`:1659`) | PARTIAL | No `RESTING` event and no margin-**clearance** event, both deliberate. Amend, cancel and transfer refusals **never reach the cursor** |
| Multi-venue in one session; `pool_for_venue` segregation | `types.py::POOL_BY_VENUE` (`:180`), `types.py:180`, `AccountRef` at `types.py:1663-1699` | IMPLEMENTED + SOURCED | Segregation itself is rulebook 6.3, HIGH. **`AccountRef` is the object that *carries* it** — `venue_scope: FrozenSet[Venue]` and `serves(venue)` (`:1697-1699`) — so the sell path can ask which pool an instrument belongs to rather than re-deriving it. The two-leg HSX + VN30F case is pinned by a **test**, which is not a source |
...
| **Three documented subsystems are not on the package's public API** | `session/__init__.py:103-146` | PARTIAL | `hasattr(plutus.market.session, …)` is **False** for `CorporateActionEngine`, `CorporateAction`, `CorporateActionAudit`, `CommissionSchedule`, `assess_daily`, `assess_at_maturity`, `DailyTurnover`, `ChargeContext`, `LeviedCharge`, `ProbabilisticFillPolicy`, `compare_policies`, `probabilistic_sweep`, `DivergenceReport`, `FillQuestion`, `SettlementResolver`, `ChargeBasis` — **and, added 2026-08-27, the whole depth subsystem: `BookWalkFillPolicy`, `SweptFillDecision`, `OptimisticQueue` (`plutus.market.session.book_walk`) and `DepthSource`, `DepthBook` (`plutus.market.adapters.depth`), none of which is on `plutus.market.session` or `plutus.market.adapters`.** That matters more than the rest of this row, because `build_fill_policy` **refuses** to construct a book-walk policy by design (§14), so the submodule import is the *only* way to reach it and a user who does not know the path cannot use the feature at all. **Import them from the submodule**, e.g. `from plutus.market.session.fills import ProbabilisticFillPolicy`. (`build_fill_policy` **is** exported.) Re-derive with `PYTHONPATH=src python -c "import plutus.market.session as S; print(hasattr(S,'BookWalkFillPolicy'))"` |
| `from_config` / `from_mapping` / `build` | `:1014`, `:1038`, `:1052` | PARTIAL | `build` wires `on_terminal` for **both** pools; a hand-assembled book that omits it leaks reservations |
| `load_data_source` from a dotted config path | `:771` | NOT BUILT (broken) | **Neither shipped adapter can be constructed from a config**, and the `DataHubSource` case fails *silently* at construction. No test covers it |
| `source=None` "admission-only study" | `::load_data_source` (`:771`) | NOT BUILT (broken) | Admits **zero** orders — but at **gate 0, not gate 7**. On the ship default (`source=None, listings=()`) `SymbolRouter.venue()` (`rulebook.py:2585`) cannot classify the ticker, so `submit()` returns `Rejected(SESSION_SEMANTICS, INDETERMINATE)` with `reason="no venue for 'FPT' … no dated listing, not a futures code, and no data source"` and the fabricated state at `exchange.py::_market_state` (`:2328`, the `MarketState(...)` at `:2339`) is never reached. `BAND_LIMIT`/INDETERMINATE is what you get **only after** supplying `listings=` — both cases re-run and confirmed |
| Event-driven callbacks | — | DEFERRED | §16. Synchronous is simpler to test and reason about |
| Immediate families decided **one interval late** | `::_decide_immediates` (`:3289`) | PARTIAL | Declared Tier-1 deviation — `submit()` is synchronous with no matching engine behind it |

---

## 14. Data contract and adapters

`market/adapters/` (**1,725 lines** — `base.py` 39, `datahub.py` 511, `depth.py` 1,002, `tick.py` 166;
count with `wc -l src/plutus/market/adapters/*.py`. The old figure of 701 predates `depth.py`).
Protocol has three methods; the session uses two — **plus the optional `IntervalSource` seam**,
which `DataHubSource` implements as of 2026-08-27 and `TickSource` does not.

> **TWO STANDING CORRECTIONS. Read both before quoting any row in this section, and do not
> let either be reintroduced anywhere in this file.**
>
> **(1) `DataHubSource` is OUTDATED and is slated for reimplementation** — the author's
> decision, 2026-08-27. Every limitation below that is really the adapter's must say
> **"the adapter"**, by name. None of them is a limitation of the design, of the rulebook,
> or of the corpus, and this file has misattributed adapter gaps to the corpus at least
> twice before (§16.4 item 4 is one; the size claim in (2) is the other). When a row here
> reads as a fact about Vietnamese market data, check whether it is really a fact about
> `datahub.py`.
>
> **(2) THE CORPUS DOES CARRY ORDER-BOOK SIZES. Any statement that computing our own
> clearing price is impossible for want of sizes is FALSE — it is a fetch, not a
> limitation.** Verified first-hand 2026-08-27 by reading the files:
> `dataset/hermes-dev-extract` holds **1,390,914 size rows across four Parquet files, every
> one carrying a `depth` column** — `local_quote_bidsize` 288,398, `local_quote_asksize`
> 233,673, `quote_bidsize` 470,850, `quote_asksize` 397,993. Reproduce with
> `select count(*) from read_parquet('<extract>/*size.parquet')`. **Production is reported
> at 666M / 590M rows; that figure is carried from the session handoff and was NOT measured
> here — grade it accordingly.** What *is* true, and is the whole of what was ever true, is
> that the two roots
> **the two shipped adapters read** are empty of sizes: `hermes-parquet`'s
> `quote_bidsize`/`quote_asksize` are **0-row** (measured), and `hermes-csv`'s are
> **header-only, 1 line each** (measured). So `datahub.py:193-195` — *"`quote_asksize` /
> `quote_bidsize` are 0-row in both corpus roots so even the tick adapter cannot supply
> sizes"* — is a **true statement about DataHub's own corpus root and a false one about the
> data**, and `tick.py:131`'s *"the size tables are 0-row in every corpus"* is false as
> written. Both are code edits this pass did not own. `adapters/depth.py` already reads the
> extract and serves sizes, so the fetch has been made for four windows.
...
| `DataHubSource` — daily, DuckDB/Parquet, **and now an `IntervalSource`** | `datahub.py::DataHubSource`, `::interval` | **PARTIAL — and OUTDATED, slated for reimplementation** (author's decision, 2026-08-27) | ⚠️ **Every limitation in this row is the ADAPTER's.** None of it is a limitation of the design, of the rulebook or of the corpus, and this file has twice made that mistake: §16.4 item 4 (`quote_open`/`max`/`min` "missing") and the size claim in standing correction (2) above. When something here reads like a fact about Vietnamese market data, check `datahub.py` first. Only `for_root(data_root)` constructs correctly from a string. `state_at` returns the **first** row of the day, ignoring the time. **Added 2026-08-27:** `interval()` serves `quote_dailyvolume` by LEFT JOIN alongside close/ceiling/floor/reference, and the interval's `MarketState` comes from the *same* `_build_state` as `state_at`, so admission and fills cannot disagree about the band, lock or phase. The contract is class data — `SERVES` / `WITHHELD` — and `WITHHELD` is stamped into `interval.missing` on **every** bar, plus `VOLUME` per bar where the corpus has no row. **Nothing defaults to zero**: an absent row is our ignorance, a zero would be a market fact, and there are zero `quantity = 0` rows in the table at any date. Non-daily resolution or a multi-day span **raises** rather than attributing a day's liquidity to a shorter window. **Coverage, measured 2021-01-01..2022-12-31: 523,619 of 832,752 ticker-days carrying a close also carry a volume row (62.9 %)** — the gap is concentrated in instruments with no volume to publish (every index is 0/499) and every liquid name checked (ACB, FPT, HPG, MWG, SSI, VIC, VN30F2211/2212, E1VFVN30) is 499/499. `quote_open`, `quote_max` and `quote_min` are still **withheld**, deliberately: wiring the extremes moves decisions in *both* directions and deserves its own measurement |
| `TickSource` — tick, 3-level book ladder | `tick.py:40` | PARTIAL | `MAX_DEPTH = 3`; **`BookLevel.size` is always `None`** — ~~the size tables are 0-row on every corpus~~ **CORRECTED 2026-08-27: that generalisation is false, see standing correction (2) above.** The true statement is narrower and is about **this adapter's root**: `hermes-csv`'s `quote_bidsize.csv` / `quote_asksize.csv` are header-only (1 line each, measured), so `_at` (`tick.py:128-136`) has nothing to read and returns `BookLevel(price=…)` with no size. `dataset/hermes-dev-extract` carries **1,390,914** size rows with a `depth` column and `adapters/depth.py` reads them. `DataField.BOOK_SIZE` exists to name the absence honestly; what it must **not** be read as is a property of Vietnamese order-book data. Also note `tick.py:131`'s own docstring still asserts the false general form |
| **`DepthSource` — the depth seam: a reconstructed order book with SIZES, and the age of every level in it** | `adapters/depth.py` (1,002 lines) | IMPLEMENTED + SOURCED (`[design]`) / measured | **Absent from this file until 2026-08-27 and it should never have been.** This is the adapter that ends the "no sizes" era: it reads `dataset/hermes-dev-extract` (four windows — FPT/HPG/VN30F2504 2025-04, HTV 2022-11) and serves a three-level two-sided ladder with quantities. Four measured corpus properties drive its whole design, and each is a refusal rather than a fill-in: **depth is 1–3 and never extrapolated to 4**; **the two sides are never observed at the same instant**, so a book is an as-of join per side and `DepthBook.cross_side_skew` reports the gap on every book (FPT 2022-11-09 median 8.3 s, p90 51.6 s, max 529.3 s; thin HTV median 584.9 s, max 13,785 s); **the tables are per-level CHANGE STREAMS, not snapshots**, so the join is per `(side, level)` and each level carries its own age (75,965 bid-size instants in the extract have a non-prefix depth set); and **price and size are not always written together** (FPT bid 2.28 %, HPG 0.16 %, HTV 18.97 % of price rows unmatched), so `size_age > price_age` marks exactly the levels where a stale size may be paired with a new price. **The reconstruction is sound in the continuous session and unsound in the auction, measured:** 4 crossed books in 7,022 continuous instants (0.06 %) against **15 of 26 (58 %)** inside 09:00–09:15 — an independent confirmation that sweeping is a continuous-session mechanic and that this corpus cannot pretend to an auction book. What it **cannot** say: there is **no deletion record**, so `SideAvailability.EMPTY` is declared and **never returned** — "the book has no shares" is a fact the data cannot express and collapsing it into "we have not seen the book" is the confusion the enum exists to prevent |
| **`BookWalkFillPolicy` — the sweep** | `session/book_walk.py` (1,592 lines) | IMPLEMENTED + SOURCED | **Also absent from this file until 2026-08-27.** The first thing in the package that lets an order **consume** a level: a limit priced through the touch is marketable, does not join the queue, and takes `min(remaining, size)` at levels 1, 2, 3 with **each tranche priced at the resting level's own price** — which is not a modelling choice but the same QĐ 352 Điều 6.3 rule `fills.py` already cites, applied per level instead of once. `BookWalk` carries a **tuple of tranches** and `SweptFillDecision` exists because `FillDecision` carries one price and cannot represent a multi-price fill. **Deliberately not config-buildable:** `build_fill_policy` registers `BOOK_WALK_KIND` only so the refusal can name it, and raises with construction instructions (`fills.py:2259-2276`), because it needs a book provider and a queue assumption `FillPolicyConfig` has no field for and defaulting either would be the silent substitution that function exists to refuse. Handed in directly: `ExchangeSession.build(..., fill_policy=BookWalkFillPolicy(DepthSource(root), queue=…))`. **130 tests** across `tests/market/session/test_book_walk.py` and `tests/market/test_depth_adapter.py` (`pytest … --collect-only`), split into a synthetic tier and a **measured** tier pinned to the extract |
...
| `MarketInterval` synthesis when no `IntervalSource` exists | `exchange.py::_interval_for` (`:2398`; the synthesis is `:2442-2451`) | PARTIAL — **D71 (the tick-path ATC) FIXED 2026-08-29** | Volume, open, high, low, book and settlement price are always `None`, so `hard` is INDETERMINATE wherever it would fill and every expiry settles on `CLOSE_PROXY`. **Scope this to sources that are not `IntervalSource`** — `DataHubSource` is one. **`close` is the exception and the problem:** it is set to `state.last` and named missing *only* when `state.last` is `None`, so in an auction phase the interval hands `auction_fill_price` a **pre-auction print** under the name of the published close. That was D71: the tick path did not implement the stated close-as-ATC approximation. **FIXED 2026-08-29 (commit `74e667a`)** by making `CLOSE` behave the way `OPEN` already does here — named missing whenever the interval is synthesised in an auction phase (`exchange.py:2648-2668`), so the ATC now returns the published close when one exists and degrades to INDETERMINATE when it does not |
| Session phase from data | `datahub.py:436`, `tick.py:109` | NOT BUILT | Both adapters **hardcode `CONTINUOUS`** (A40). *`datahub.py:210` was the recorded site and is stale — `grep -n "session=" src/plutus/market/adapters/datahub.py` returns **436** and nothing else.* **Scope this claim to the adapters** — it is accurate about them, and it does *not* mean ATO/ATC/PLO are unreachable **for admission**: on any non-daily resolution `exchange.py:2081-2101` takes the phase from `RuleSet.phase` and the adapter is only the `UNKNOWN` fallback. Executed at tick resolution: 09:05 HSX `opening_auction` / HNX `continuous`; 12:00 both `noon_break`; 14:35 both `closing_auction`; 14:50 HSX `post_close` / HNX `post_close_plo`; Saturday → `post_close`. **And the reassurance's old caveat is WITHDRAWN, 2026-08-27 (later).** This row said *"no ATO or ATC has ever crossed on a shipped source — the auction fill is built, tested and unreachable from `src/`"*. That is wrong: `_evaluate_fills` re-stamps the state with the rulebook phase at `exchange.py:2841-2843`, so on a **tick** run the ATC branch of `auction_fill_price` **does** fire — and formerly filled at `state.last`, a pre-auction print, the defect **D71**, FIXED 2026-08-29 (commit `74e667a`): a synthesised interval in an auction phase now drops the close (`exchange.py:2648-2668`), so the ATC returns the published close or INDETERMINATE. The ATO branch still never fires, correctly, because a synthesised interval has no `open` (A75 explains why there is nothing to put there). `validation/scenarios/bars.py:310` (`PhasedBarSource`) is still the only source outside `src/` that stamps a real phase, and its `:375-381` warning about admission judging in one phase and the fill in another is now **realised on the tick path** rather than hypothetical |
...
| Feature | Where | Status | Source / note |
|---|---|---|---|
| `VsdcSettlementCalendar` — dates are a **caller-supplied data input**, no hardcoded holidays | `calendar.py:140` | IMPLEMENTED + SOURCED | Reproduces the rulebook's Announcement 4228/TB-VSDC Tết-2026 worked example **when the notice is loaded into it** (`from_file`) — not by default, and not from the corpus, whose coverage ends before 4228 is dated. Queries outside coverage **raise**. A64 is the resolved statement of what a default run gets |
...
| **Queue-position matching** against a reconstructed book | ~~81 % of best-quote changes carry no trade and there are no order ids, so order flow cannot be recovered. `Probabilistic` is the honest ceiling~~ — **the reason is half spent, 2026-08-27.** Our own queue *position* is still unrecoverable and `BookWalkFillPolicy` therefore takes an explicit `queue=` assumption instead of inferring one. The *book* is not: `adapters/depth.py` serves a three-level sized ladder and `session/book_walk.py` sweeps it. Restate the deferral as **"queue position, not depth"** | design §13, `fills.py:225`; §14 |
...
| **Put-through trading** | Out of scope; `Side.CROSS` raises rather than being silently mis-modelled | `exchange.py::_reserve` (`:2578-2583`) |
...
| **Amend price / amend-up** | Re-taking a reservation on the same key is refused by design, and release-then-retake can fail after the release and leave a live order unfunded | `exchange.py::amend` (`:1599`) |
...
18. **Twenty-one documented objects are absent from the package's public API** (§13) —
    sixteen from `plutus.market.session`, plus the whole depth subsystem added
    2026-08-27: `BookWalkFillPolicy`, `SweptFillDecision`, `OptimisticQueue`,
    `DepthSource`, `DepthBook`. Mostly a documented feature whose import path the doc did
    not state — **except for the book-walk policy, where it is closer to a real gap**:
    `build_fill_policy` refuses to construct one from a config on purpose, so the
    submodule import is the only route to the feature and a user who does not know it
    cannot reach the sweep at all.
...
26. **`adapters/tick.py` serves no `MarketInterval` at all.** `DataHubSource` gained the
    ...
    have to start.~~ **Corrected 2026-08-27:** it is **not** the only such adapter and the
    depth-aware policy did not start there. `adapters/depth.py` reaches the ladder *with
    sizes* and `session/book_walk.py` sweeps it (§14). What survives is the narrow gap: the
    tick adapter has no `interval()`, and **that absence was load-bearing in the wrong
    direction** — it routed a tick run into the interval synthesis that
    caused **D71** (FIXED 2026-08-29: that synthesis now names `CLOSE` missing in an auction
    phase, so the ATC returns the published close or INDETERMINATE).
...
2. **RESOLVED 2026-08-29 (MUST #4) — `settle_daily` is wired and now pays daily VM cash.**
   ~~`settle_daily` has no session call site, so VM is cumulative rather than daily~~ (D1).
   The author's call was made: cash-settle both regimes. `settle_daily` now moves the day's
   P&L as cash (`_move`) **and** rolls the baseline, called from `exchange.py::_overnight_margin`
   (`:4026`) once per settlement day before the requirement is read — its docstring no longer
   says "no cash moves", and there is now code in `src/` that pays daily VM cash. This closes
   **both** the cumulative-vs-daily half and the missing-cash half. Historical measure of the
   gap it closed: at one corpus mark the un-wired model reported utilisation 1.2099 where
   VSDC's actual daily cash settlement gave 1.371 — the simulator was consistently later to
   call than a real clearing member; the cash leg removes that lag.
...
4. **Every price in every corpus scenario is same-session look-ahead.**
   ...
   **close**, and `_marks()` prices the 09:30 margin mark with it. Consequences measured:
   ...
   of them — **`DataHubSource`'s gap, by name.** `exchange.py::_interval_for` (`:2442`)
   unconditionally declares them missing, which is that adapter gap **misattributed to the
   corpus**. This is the single highest-value remaining fix and it is an adapter change
   that moves every number in every scenario, so it wants its own pass and the author's
   sign-off. **2026-08-27:** the author has decided `DataHubSource` is outdated and is to
   be **reimplemented**, so this item is now a requirement on that rewrite rather than a
   patch to the current file. The same misattribution shape recurred with order-book
   sizes — see §14 standing correction (2), where the corpus turned out to hold
   1,390,914 of them.
...
12. **RESOLVED 2026-08-29 (MUST #4) — the variation margin is now paid in cash.** Post-KRX
    `MR` is smaller than pre-KRX `MR` by exactly the variation margin (measured: 60,044,000đ
    against 109,844,000đ on one 2-lot VN30F position), and Phụ lục 2 §6.2 has no `VM` term
    because Điều 20 settles position P&L as a separate T+1 cash movement. Conflict 2 above —
    that the simulator did **not** make that movement — is now closed: `settle_daily` pays the
    day's P&L in cash before the grid is read, so the view carries `VM == 0` and
    `margin.overnight.assumed.variation_margin_unsettled` is no longer raised except in the
    residual case of a held contract with no settlement price that session. The mixture was
    permissive — a run quoting the grid's number while carrying the loss in nothing at all
    reported an account owing less than it did — and that permissiveness is removed in the
    settled case.
...
| # | Defect | Where |
|---|---|---|
| D1 | ~~`settle_daily` is documented as the DSP rebaseline but **no session path calls it**, so VM is measured from `average_entry` for the life of every position (A60)~~ **RESOLVED 2026-08-29 (MUST #4).** `settle_daily` is now wired into `exchange.py::_overnight_margin` (`:4026`): once per settlement day, before the requirement is read, it moves the day's P&L as cash (`_move`) **and** rolls the baseline (`_settlement_reference`), both regimes — so VM is the day's move from the previous DSP, not cumulative-since-entry, and A60 resolves with it | `deposit.py::settle_daily` (`:1262-1317`), `exchange.py:4026` |
| D2 | `ExchangeSession.transfer`'s docstring says the deposit withdrawal is bounded by `free_deposit`; it is bounded by `balance − MR/threshold`, which `deposit.py` explicitly says is **not** `free_deposit` | `exchange.py::transfer` (`:1763`) vs `deposit.py:1113` |
...
| D5 | `_session_refusal` stamps `INDETERMINATE` purely on `phase is UNKNOWN`, mislabelling three **definite** refusals (already-terminal, non-amendable type, quantity-below-filled) as ignorance | `orders.py::_session_refusal` (`:1225`) |
...
| D7 | A **third** charge engine (`exchange.py::_derivative_charges`, `:3190`) duplicates `ledgers.assess_charges`, and its stated reason is now stale — `assess_charges` grew the `multiplier` parameter it asked for, and returns identical amounts. A live drift risk | `exchange.py:3190` |
...
| D10 | `expires_at_boundary` docstring says an ATO dies at "its own cross"; the code keys on `tif` and fires at the end of **any** auction phase | `orders.py::expires_at_boundary` (`:279`) |
| D11 | `amend()`'s defaults `allow_price_and_quantity=True` / `priority_preserving=True` encode the pre-2025-05-05 regime and the **rejected** side of a recorded CONFLICT. **Pointer corrected 2026-08-27:** `def amend` is `orders.py:935`, `allow_price_and_quantity: bool = True` is `:943` and `priority_preserving: bool = True` is `:944`. The recorded `orders.py:901` is inside `cancel()`'s docstring, describing the `SessionPhase` argument — D11's substance was right and nobody could check it at the line given | `orders.py::amend` (`:935`, defaults `:943-944`) |
| D12 | `reject()`'s docstring claims the terminal hook may have "something to release"; the record it builds can never carry an encumbrance | `orders.py::reject` (`:684`) |
| D13 | Amend, cancel and transfer refusals **never reach the event cursor** and are counted only when INDETERMINATE | `exchange.py::amend` (`:1599`), `::cancel` (`:1570`), `::transfer` (`:1763`) |
...
| D28 | **A dangling citation repeated three times — line numbers and content both corrected 2026-08-27.** The sites are `ledgers.py:628`, `:726`, **`:1221`**; the previously recorded `:1209` carries no such citation and the catalogue inherited that wrong pointer verbatim. Verified by `grep -n "rulebook 8.4" src/plutus/market/session/ledgers.py` → **628, 726, 1221**, exactly three hits. **"Rulebook 8.4 does not exist" is correct**: §8 runs 8.1 Taxes (`:894`) / 8.2 Exchange and depository prices (`:923`) / 8.3 Brokerage commissions (`:971`) and stops at §9 (`:995`). **The content claim splits two ways, and not the way an audit proposed** — read from the code, the three sites do *not* all cite TT 121/2020 Art. 27: **(a) `:628`** is inside `AdvanceTerms`' docstring and attaches "rulebook 8.4" to *"a brokerage-licensed firm may offer the service with prior written SSC approval (**Luat Chung khoan 54/2019 Art. 86(1)(b)**)"* — the legal-status row, rulebook **§5.2:541**, graded **`high`** (*"confirmed word for word"*). Note the *same sentence* already cites **rulebook 8.3** correctly, with its own *high (structure)* grade, for the self-priced half; only the 86(1)(b) pointer is dangling. **(b) `:726` and `:1221`** both cite *"a Vietnamese securities company may not lend money (**TT 121/2020 Art. 27**)"* — rulebook **§5.2:542**, graded **`low`** (*"read in summary form only, not verbatim"*). So the fix is 8.4 → **5.2** at all three sites, with **one** of them inheriting `high` and **two** inheriting `low`. Carry the `low`. (The §5.2 line was recorded here as `:511`; it is **`:542`** in the current rulebook — also corrected) | `ledgers.py:628`, `:726`, `:1221` |
...
| D31 | **RESOLVED 2026-08-29 — archived, not deleted.** The five unimportable `core/` modules (`bot`, `algorithm`, `portfolio`, `position`, `transaction`) were the pre-exchange-model trader stack; `position.py`/`transaction.py` did a bare `import utils` that only ever resolved to **`tests/utils.py`** under pytest's prepend-import mode, so the suite was green only because `tests/utils.py` shadowed the gap. They have been moved to **`archive/legacy-trader-stack/`** — a directory with **no `__init__.py`**, so nothing can import them and they are not shipped in the wheel. The one test that imported `bot` (`tests/datahub/test_daily_ohlc.py`) was removed; `protocol.py` and `plutus/__init__.py` no longer point at them. `market.protocol.Position` (`protocol.py:164`) remains the live class. | `archive/legacy-trader-stack/` |
| D32 | **The withdrawn Article-13 attribution survives in three modules this correction did not own.** `broker.py` and `deposit.py` were corrected on 2026-08-26; the identical claim is still live at `margin.py:12-14` (*"an 80/90/100 ladder … Article 13 of QĐ 96/QĐ-VSD → QĐ 61/QĐ-VSD, **confidence high**"* — note this one stops the chain at QĐ 61 and grades it HIGH anyway), `margin.py:75` (`PROVENANCE`, *"the actual call test is utilisation = MR / margin assets against an 80/90/100 ladder"*), `exchanges/derivatives.py:115`, and — the one most likely to be copied — **`types.py:649-658`, `MarginStatus`'s own docstring**, which still says *"the shape of the ladder is VSDC-sourced (rulebook 6.3, Article 13: levels 1/2/3 at 80/90/100 per investor account)"*. Behaviour is unaffected in all four; the **claim** is dead. Reword to match `BrokerTerms.PROVENANCE` | `types.py:657-658`, `margin.py:12`, `:75`, `exchanges/derivatives.py:115` |
...
**D71 was found later the same day, by the auction-fidelity read**, and it is a different kind
of entry from every other row in this file: it is **live, user-visible and one condition from
being closed**. It is not fixed here because this pass owned the register and not the code.
Nothing else in §17 currently puts a wrong number in a user's hands on the shipped tick path.
...
| D71 | **✅ FIXED 2026-08-29.** The fix is the one condition the row below predicted: `exchange._interval_for`, in a synthesised interval whose phase is an auction, now sets `close = None` and adds `DataField.CLOSE` to `missing` — so the ATC returns the published close where an `IntervalSource` supplies one and INDETERMINATE on a bare snapshot, symmetric with the ATO. Expiry settlement is untouched — not because it skips `interval.close` (`_final_settlement` reads it first, then falls back to `state.last`), but because the settlement interval carries the raw observed state, not the fill path's rulebook re-stamp, and shipped adapters stamp `CONTINUOUS`, so the auction branch never fires there; and if it did, the dropped close was only ever `state.last`, the exact value the fallback supplies. Guarded by `tests/market/session/test_exchange.py::test_d71_tick_atc_does_not_fill_at_the_stale_last` and demonstrated by scenario **J37** (`scenarios/test_j37_tick_atc_published_close.py`). The original diagnosis, kept for the record: **THE TICK PATH DOES NOT IMPLEMENT THE STATED CLOSE-AS-ATC APPROXIMATION. A live, user-visible design-conformance defect** — found 2026-08-27 by reading the code, not inferred, and every link below re-verified line by line. **The stated model (A75)** takes the day's **published close** as the ATC outcome; on a tick run the code instead fills the ATC at `state.last`, a **stale pre-auction print**. **The chain.** (1) At 14:35 on a **tick** run `_evaluate_fills` does the right thing: `exchange.py:2841` resolves `phase = self._phase(record.venue, observed=state.session)` and `:2842-2843` re-stamps the state with it — and on any non-daily resolution `_phase` (`:2081`) returns `self._rulebook.at(self._now).phase(venue)` at `:2099`, so the interval's phase is the **rulebook's** `closing_auction`, not the adapter's `CONTINUOUS`. (2) `TickSource` is **not** an `IntervalSource` — it has `states`/`state_at` and no `interval()` (`tick.py:55`, `:159`; the protocol is `exchange.py:573`) — so `_interval_for` falls past `:2437-2441` to the synthesis at **`:2442-2451`**, which sets `missing = {OPEN, HIGH, LOW, VOLUME, BOOK_SIZE}` (`:2442-2443`), `close = state.last` (`:2444`), and adds `CLOSE` to `missing` **only when `state.last is None`** (`:2445-2447`). (3) `auction_fill_price` (`fills.py:608-636`) then passes the `phase is interval.session` gate at `:632` and returns `interval.close` at `:636`, under a docstring promising *"the published open (ATO phase) or close (ATC phase)"*. **Continuous matching stops before the ATC and the cross publishes at 14:45, so `state.last` is a pre-auction print — the fill is a pre-auction price wearing the auction's label, which is not the published close the stated model promises.** **The fix is one condition, and the other half of the same function proves it correct:** `OPEN` is *always* in `missing` and a synthesised interval never sets `interval.open`, which is exactly why the opening-auction branch already returns INDETERMINATE and is already right. Make `CLOSE` behave like `OPEN` — named missing whenever the interval is **synthesised** and the phase is an auction — and the ATC branch returns the published close when one is available or degrades to INDETERMINATE when it is absent, which is what the stated approximation calls for. **Scope, so nobody over-reads it:** the DAILY path never fires at all — `DataHubSource` stamps every bar `CONTINUOUS` (`datahub.py:436`) and, being an `IntervalSource`, returns its **own** interval at `exchange.py:2441`, silently discarding the `:2843` re-stamp; `_auction_phase` (`fills.py:2445`) then requires `phase is interval.session` and fails. **That discard is a second, latent hazard**: the moment any adapter serves intervals at tick resolution, the rulebook phase computed at `:2841` stops reaching the fill seam again | `exchange.py::_interval_for` (`:2442-2451`), `fills.py::auction_fill_price` (`:608-636`) |
...
6. **Line numbers drift, and they drift in bulk.** They are a navigation aid, not a
   contract. If a `file:line` no longer points at what the row says, fix the line number —
   do not delete the row. **Two habits, learned 2026-08-27 the expensive way.** (a) Prefer
   `module::symbol`, with the line as a parenthetical: a symbol does not drift, and this
   file's newer rows already use that form. (b) **Sweep, do not sample.** Two stale pointers
   found by sampling turned out to be two of about forty-five: `exchange.py` had grown by
   ~175 lines and essentially every pointer into it past `:800` was wrong, several landing
   on a blank line, a `}`, or prose about an unrelated subject. Resolve *every*
   ``` `foo.py:N` ``` in this file against `src/` and print the line before a release.
...
`exchange.py::_interval_for` (`:2442`) declares them missing — an adapter gap misattributed to the corpus.
...
### 2026-08-27 (later still) — the auction-fidelity read, and a pointer sweep

**Four things were asked of this file and all four changed something.**

**1. Two stale code pointers, and they were not alone.** `FEATURES.md` cited
`exchange.py:1306` in §1 and §6 for the price-and-quantity-together ban; that line is the
**settlement-calendar** docstring (the Tết example A64 quotes) and the real site is
`_may_amend_price_and_quantity` at **`:2108`**, citation at `:2112-2114`. D11 cited
`orders.py:901` for `amend()`'s defaults; `def amend` is **`:935`**, the two defaults are
`:943` and `:944`, and `:901` is inside `cancel()`'s docstring. Both instruments were
right; only the pointers were stale — §18 rule 6's exact shape.

**The sweep found the drift is systemic, not two typos.** All 282 `file:line` pointers in
this document were resolved and printed against the current source. `exchange.py` has grown
to **4,041 lines** (this file said 3,866) and essentially every pointer into it past
`:800` was wrong — `submit` `:811`→`:1453`, `advance_to` `:750`→`:1362`, `_reserve`
`:1572`→`:2512`, `_sweep_non_resting` `:1796`→`:2746`, `_derivative_charges`
`:2118`→`:3190`, and about thirty more. `orders.py` drifted the same way
(`_terminate` `:550`→`:592`, `_session_refusal` `:1130`→`:1225`, `expire_due`
`:1038`→`:1080`). **Roughly 45 pointers were repaired, and the repair converts them to
`module::symbol` form with the line as a parenthetical**, because a symbol does not drift
and this file already used that idiom in its newer rows. The 18 `FillDecision.indeterminate`
sites were re-derived rather than trusted (still 18, all sixteen line numbers changed).

> **Reproduce the sweep before the next release.** Resolve every ``` `foo.py:N` ``` in this
> file against `src/` and print the line. Anything whose printed line is a blank, a `}`, or
> prose about a different subject is stale. Two of the three worst offenders were found by
> *sampling* — assume any unswept pointer is wrong until printed.

**2. A64 versus the checklist's RESOLVED section — settled from the code, in A64's
favour.** Three greppable facts, now in A64 itself: the default really is
`weekday-only-UNSOURCED` (`exchange.py:1331` → `weekday_settlement_calendar()`, empty
holiday set, `is_sourced == False`); `from_settlement_days` has **zero callers in `src/`**,
so the "derived" path is a harness route and not a default; and the derived path does **not**
reproduce Announcement 4228/TB-VSDC on its own terms — `validation/scenarios/settlement.py`'s
own `CALENDAR_SOURCE` calls the measured closure set *"an INFERENCE, not a VSDC notice"* and
says it *"inherits none of that grade"*, 4228 being dated 2025-11-20 against a corpus ending
2022-12-30. **`PUBLISH-CHECKLIST.md`'s RESOLVED entry is wrong on both halves and needs a
correction; that file belongs to another agent this phase and was not edited from here.**

**3. The auction cross — one citation withdrawn, one live defect found.**

| | Before | After |
|---|---|---|
| The auction fill price | IMPLEMENTED + **SOURCED**, sharing §7's QĐ 352 Điều 6.3 citation | IMPLEMENTED + **ASSUMED** — **A75**, our modelling choice: the day's published open/close, adopted because auction-window tick data is untrustworthy and both are already stored. Điều 6.2(a)–(d) derives a cross *from a book*; Điều 6.3 is the *continuous* rule; we cite neither for the price |
| Can an ATC cross on a shipped source? | A40 and §14 said **no**, ever | **Yes, on tick — and since 2026-08-29 it implements the stated model.** `exchange.py:2841-2843` re-stamps the rulebook phase, so `auction_fill_price` returns `interval.close`; the synthesis now sets that to `None` and names `CLOSE` missing in an auction phase (`exchange.py:2648-2668`), so the ATC returns the published close where a source supplies one and INDETERMINATE otherwise. **D71, FIXED** |

The price is **our modelling choice, not a rule**: we do not trust the tick data inside the
auction window, so we take the day's published close as the ATC outcome and the day's
published open as the ATO outcome, both already stored. As *context* only, QĐ 352 **Điều
2.5** — *"Giá đóng cửa là giá thực hiện tại lần khớp lệnh cuối cùng trong ngày giao dịch"*,
the close is the day's last match — is why the published close is a fair stand-in for the
ATC. There is separately no opening price in Vietnamese exchange regulation at all
(`grep -c "mở cửa"` over the rulebook returns **1**, and that one hit is about HNXDS opening
15 minutes early, a time and not a price), so the ATO half rests on a stored vendor field.
A75 carries the whole statement.

**D71 was the only row in §17 that was live, user-visible, and one condition from closed** — a
design-conformance defect: the tick path filled the ATC at `state.last` instead of the
published close the stated model promises. **It is now FIXED (2026-08-29):** the one condition
landed in `exchange._interval_for` (drop the close, name it missing, in a synthesised auction
phase), guarded by `test_d71_tick_atc_does_not_fill_at_the_stale_last` and demonstrated by J37.
§17 has no live user-visible defect remaining.

**4. Two standing corrections landed, and one of them uncovered two undocumented modules.**
`DataHubSource` is **outdated and slated for reimplementation** (author's decision), and
§14 now opens with that plus the instruction to attribute adapter gaps to *the adapter*, by
name. The order-book-size claim was checked against the files rather than repeated:
`dataset/hermes-dev-extract` holds **1,390,914 size rows across four Parquet files, each
with a `depth` column** (288,398 / 233,673 / 470,850 / 397,993), while `hermes-parquet`'s
size tables are **0-row** and `hermes-csv`'s are **header-only**. So the claim was true of
the two roots the two shipped adapters read and false about the data, exactly as
§16.4 item 4's `quote_open` misattribution was. **Computing our own clearing price is a
fetch, not a limitation.**

Chasing that turned up **`adapters/depth.py` (1,002 lines)** and
**`session/book_walk.py` (1,592 lines)** — a sized three-level book source and a
level-by-level sweep, **130 tests between them**, neither mentioned anywhere in this
document. Both now have rows in §14, and the deferrals they retire (§7 and §16.2's
queue-position rows, §16.3 #26) have been restated as **"queue position, not depth"**.
...
the header now records per-suite counts (market 1646 / scenarios 38 / strategies 13) with the
...
subprocess tests a bare pytest reported as 17 failures pass under the venv (a PYTHONPATH
collision, not a logic failure).
