...
- **⚠️ ONE LIVE, USER-VISIBLE DEFECT: [D71](#17-known-defects-and-docstring-mismatches).** The
  stated model takes the day's **published close** as the ATC outcome — our approximation,
  because auction-window tick data is untrustworthy and the close is already stored (A75).
  On a **tick** run the code does not implement that: an ATC order fills at `state.last`, a
  **stale pre-auction print**, instead of the published close. **The fix is one condition**
  and D71 names it — an ATC fill must return the published close, or INDETERMINATE when the
  close is absent. Nothing else in §17 currently puts a wrong number in a user's hands
...
**D71 is the only row in §17 that is live, user-visible, and one condition from closed** — a
design-conformance defect: the tick path fills the ATC at `state.last` instead of the
published close the stated model promises. It is not fixed here because this pass owned the
register and not the code.
