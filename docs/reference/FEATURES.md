...
- **✅ [D71](#17-known-defects-and-docstring-mismatches) FIXED (2026-08-29) — no live,
  user-visible defect remains.** The one live defect was D71: on a **tick** run the ATC filled at
...
- **Suite:** `python -m pytest -q` → **2,449 collected, 2,432 passed, 17 failed**,
  2026-08-27 (re-measured; supersedes the stale "2308 passed" this header used to show).
  **The 17 failures are all in `tests/datahub/test_cli.py`** — a PYTHONPATH / module-name
  collision at collection time, **not a logic failure** in the code under test. The
  collected total reconciles fully (2,432 + 17 = 2,449, nothing silently dropped) and now
  includes `tests/market/session/test_book_walk.py` and `tests/market/test_depth_adapter.py`
  (130 tests between them), which the prior header predated. On the collected-vs-passed gap
  recorded as **D36**: it was measured on the 1318-test tree and has not been re-measured
  since — do not quote D36's numbers against this one
...
26. **`adapters/tick.py` serves no `MarketInterval` at all.** `DataHubSource` gained the
    ...
    tick adapter has no `interval()`, and **that absence is now load-bearing in the wrong
    direction** — it is precisely what routes a tick run into the interval synthesis that
    causes **D71**.
...
| Can an ATC cross on a shipped source? | A40 and §14 said **no**, ever | **Yes, on tick — and it does not implement the stated model.** `exchange.py:2841-2843` re-stamps the rulebook phase, so `auction_fill_price` returns `interval.close`, which the synthesis at `:2444` set to `state.last`: a **pre-auction** print, not the published close the model calls for. **D71** |
...
the header now reads **2,449 collected / 2,432 passed / 17 failed** (the 17 a PYTHONPATH
collision in `tests/datahub/test_cli.py`, not a logic failure).
